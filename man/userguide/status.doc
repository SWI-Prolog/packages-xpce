...
\begin{itemize}
    \item Fonts, colours, cursors and images have been updated,
    removing many of the old legacy X11 features.
    ...
    \item The `->fill` method used to fill graphicals currently
    only accepts filling with a solid colour.  Filling with repeated
    patterns is not yet supported.  This functionality may be restored.
    ...
    \item Generating PostScript from graphicals	has been
    removed. A new method `graphical->pdf` is provided that use Cairo to
    write PDF to a file. The implementation rather minimal. Notably
    producing multi-page documents is not yet supported.

    \item There is no support for printing.  The above mentioned
    `graphical->pdf` allows for generating PDF from graphicals that
    can then be printed.

    \item While internal drag-and-drop is supported, there is currently
    no support to communicate with external applications using
    drag-and-drop.  SDL does support dropping external objects, so this
    may be supported in due time.

    \item Printing and handling for Windows meta-files are currently not
    supported.
\end{itemize}
