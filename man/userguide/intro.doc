...
The main target language for \product{} is {\em Prolog} and this
document concentrates on \productpl{} rather then \product{}/Lisp.
...
sufficiently large, 64-bits memory model. \product{}'s graphical classes
(including windows, etc.) interface to the hosting platform using widely
supported portable libraries: \href{https://www.libsdl.org}{SDL},
\href{https://www.cairographics.org/}{Cairo} and
\href{https://www.gtk.org/docs/architecture/pango}{Pango}.  \product{}
is actively maintained for Linux, MacOS and Windows.
...
GTK, Qt, or Win32. Instead, it is built on top of its own base layer to
create and manipulate windows, draw primitives such as lines, circles,
text and handle user-events.
...
The ``PCE Project'' was started in 1985 by Anjo
Anjewierden\textsuperscript{\dag}. His aim was to develop a high-level
UI environment for Edinburgh (C-)Prolog. The requirements for this
environment came from the ``Thermodynamics Coach'' project in which Paul
Kamsteeg used PCE/Prolog to implement the UI for a courseware system for
thermodynamics. This system included a `scratch-pad' that allowed the
student to create structured drawings of component configurations. The
application had to be able to analyse the drawing made by the student.
...
at \url{https://www.swi-prolog.org} for details.

As of XPCE 7.0, the license terms are changed to BSD-2 and portability
of the system was handed to the widely supported libraries:
\href{https://www.libsdl.org}{SDL},
\href{https://www.cairographics.org/}{Cairo} and
\href{https://www.gtk.org/docs/architecture/pango}{Pango}.  \product{}
was extended to provide the ``app'' Prolog console (named
\textit{Epilog}).  \product{}~7 provides more flexible support for
Prolog's multi-threading.
\end{xpceonly}


\section{About this manual}

This userguide introduces the basics of \productpl{} and its
